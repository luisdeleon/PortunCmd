...
const countriesWithStates: Record<string, StateProvince[]> = {
  US: [
...
export const useCountriesStates = () => {
  const getCountries = () => allCountries

  const getStatesForCountry = (countryCode: string): StateProvince[] => {
    return countriesWithStates[countryCode] || []
  }

  const countryHasStates = (countryCode: string): boolean => {
    return !!countriesWithStates[countryCode]
  }
